...
def process_transaction(json_formatted_data, line, transaction_regex, desc_regex, ignorable_regexes):
    ...
    if transaction_pattern.match(line):
        json_formatted_data[constants.TRANSACTIONS_STR].append({
            ...
            constants.CLOSING_BALANCE_STR: bsr_utils.pretty_format(
...
def extract(_file, password):
    header_pattern = re.compile(constants.KARUR_VYSYA_BANK_HEADER_REGEX)
    file_end_pattern = re.compile(constants.KARUR_VYSYA_BANK_STATEMENT_END_REGEX)
    ...
    for line in file_content:
        if file_end_pattern.match(line):
            ...
            process_transaction(json_formatted_data, line, constants.KARUR_VYSYA_BANK_TRANSACTION_REGEX,
                                constants.KARUR_VYSYA_BANK_DESC_REGEX,
                                constants.KARUR_VYSYA_BANK_IGNORABLE_REGEXS)
        elif header_pattern.match(line):
            is_transaction_started = True
            bsr_utils.put_acc_details(json_formatted_data, acc_details,
                                      constants.KARUR_VYSYA_BANK_ACCOUNT_DETAILS_REGEX)
        else:
            acc_details += line + '\n'
