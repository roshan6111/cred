...
def process_desc(json_formatted_data, desc_pattern, line):
    ...
    json_formatted_data[constants.TRANSACTIONS_STR][-1][constants.DATE_STR] += ' ' + bsr_utils.pretty_format(m.group(1))
...
def extract(_file, password):
    header_pattern = re.compile(constants.IDBI_BANK_HEADER_REGEX)
    file_end_pattern = re.compile(constants.IDBI_BANK_STATEMENT_END_REGEX)
    ...
    for line in file_content:
        if file_end_pattern.match(line):
            ...
            process_transaction(json_formatted_data, line, constants.IDBI_BANK_TRANSACTION_REGEX)
        elif header_pattern.match(line):
            is_transaction_started = True
            bsr_utils.put_acc_details(json_formatted_data, acc_details, constants.IDBI_BANK_ACCOUNT_DETAILS_REGEX)
        else:
            acc_details += line + '\n'
