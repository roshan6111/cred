...
def extract(_file, password):
    header_pattern = re.compile(constants.DIGI_BANK_HEADER_REGEX)
    file_end_pattern = re.compile(constants.DIGI_BANK_STATEMENT_END_REGEX)
    ...
    while i < len(file_content):
        ...
        if file_end_pattern.match(line):
            ...
            process_transaction(json_formatted_data, line, constants.DIGI_BANK_TRANSACTION_REGEX,
                                constants.DIGI_BANK_DESC_REGEX, constants.DIGI_BANK_IGNORABLE_REGEXS)
        elif header_pattern.match(line):
            is_transaction_started = True
            i -= 1
            bsr_utils.put_acc_details(json_formatted_data, acc_details,
                                      constants.DIGI_BANK_ACCOUNT_DETAILS_REGEX)
        else:
            acc_details += line + '\n'
        i += 1
