...
def extract(_file, password):
    header_pattern = re.compile(constants.ALLAHABAD_BANK_HEADER_REGEX)
    file_end_pattern = re.compile(constants.ALLAHABAD_BANK_STATEMENT_END_REGEX)
    ...
    for line in file_content:
        if file_end_pattern.match(line):
            ...
            process_transaction(json_formatted_data, line, constants.ALLAHABAD_BANK_TRANSACTION_REGEX,
                                constants.ALLAHABAD_BANK_DESC_REGEX, constants.ALLAHABAD_BANK_IGNORABLE_REGEXS)
        elif header_pattern.match(line):
            is_transaction_started = True
            bsr_utils.put_acc_details(json_formatted_data, acc_details, constants.ALLAHABAD_BANK_ACCOUNT_DETAILS_REGEX)
        else:
            acc_details += line + '\n'
