...
def extract(_file, password):
    ...
    for line in file_content:
        if file_end_pattern.match(line):
            ...
            process_transaction(json_formatted_data, line, constants.BOI_BANK_TRANSACTION_REGEX,
                                constants.BOI_BANK_DESC_REGEX, constants.BOI_BANK_IGNORABLE_REGEXS)
        elif header_pattern.match(line):
            is_transaction_started = True
            bsr_utils.put_acc_details(json_formatted_data, acc_details, constants.BOI_BANK_ACCOUNT_DETAILS_REGEX)
        else:
            acc_details += line + '\n'
    if "name" in json_formatted_data:
        json_formatted_data[constants.NAME_STR] = json_formatted_data[constants.NAME_STR].split(' S/O ')[0]
