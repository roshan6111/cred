...
def extract(_file, password):
    header_pattern = re.compile(constants.FEDERAL_BANK_HEADER_REGEX_TWO)
    file_end_pattern = re.compile(constants.FEDERAL_BANK_STATEMENT_END_REGEX_TWO)
    ...
    while i < len(file_content):
        ...
        if file_end_pattern.match(line):
            ...
            bsr_utils.put_acc_details(json_formatted_data, acc_details,
                                      constants.FEDERAL_BANK_ACCOUNT_DETAILS_REGEX_TWO)
        else:
            acc_details += line + '\n'
        i += 1
