...
def process_transaction(json_formatted_data, line, transaction_regex, desc_regex, ignorable_regexes):
    ...
    # print (json_formatted_data (uncomment to print console result))


def extract(_file, password):
    header_pattern = re.compile(constants.ANDHRA_BANK_HEADER_REGEX)
    ...
    for line in file_content:
        if file_end_pattern.match(line):
            ...
            process_transaction(json_formatted_data, line, constants.ANDHRA_BANK_TRANSACTION_REGEX,
                                constants.ANDHRA_BANK_DESC_REGEX, constants.ANDHRA_BANK_IGNORABLE_REGEXS)
        elif header_pattern.match(line):
            is_transaction_started = True
            bsr_utils.put_acc_details(json_formatted_data, acc_details, constants.ANDHRA_BANK_ACCOUNT_DETAILS_REGEX)
        else:
            acc_details += line + '\n'
