...
def extract(_file, password):
    ...
    acc_details = ''
    transaction_pattern = re.compile(constants.INDUSLAND_BANK_TRANSACTION_REGEX)
    ignorable_patterns = [re.compile(ignorable_regex) for ignorable_regex in constants.INDUSLAND_BANK_IGNORABLE_REGEXS]
    ...
    for line in file_content:
        ...
        if bsr_utils.is_ignorable(ignorable_patterns, line):
            ...
            process_transaction(json_formatted_data, line, constants.INDUSLAND_BANK_TRANSACTION_REGEX,
                                constants.INDUSLAND_BANK_DESC_REGEX, constants.INDUSLAND_BANK_IGNORABLE_REGEXS)
            ...
            bsr_utils.put_acc_details(json_formatted_data, acc_details, constants.INDUSLAND_BANK_ACCOUNT_DETAILS_REGEX)
        else:
            acc_details += line + '\n'
