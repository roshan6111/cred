...
def extract(_file, password):
    ...
    while i < len(file_content):
        ...
        if file_end_pattern.match(line):
            ...
            process_transaction(json_formatted_data, i, constants.CENTRAL_BANK_TRANSACTION_REGEX, file_content,
                                constants.CENTRAL_BANK_DESC_REGEX)
        elif header_pattern.match(line):
            is_transaction_started = True
            bsr_utils.put_acc_details(json_formatted_data, acc_details, constants.CENTRAL_BANK_ACCOUNT_DETAILS_REGEX)
        else:
            acc_details += line + '\n'
        i = i + 1
