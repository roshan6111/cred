...
def extract(_file, password):
    header_pattern = re.compile(constants.UNION_BANK_HEADER_REGEX)
    ...
    while i < len(file_content):
        ...
        if is_transaction_started:
            pass
            process_transaction(json_formatted_data, line, constants.UNION_BANK_TRANSACTION_REGEX,
                                constants.UNION_BANK_DESC_REGEX, constants.UNION_BANK_IGNORABLE_REGEXS)
        if header_pattern.match(line):
            is_transaction_started = True
            bsr_utils.put_acc_details(json_formatted_data, acc_details, constants.UNION_BANK_ACCOUNT_DETAILS_REGEX)
        else:
            acc_details += line + '\n'
        i += 1
